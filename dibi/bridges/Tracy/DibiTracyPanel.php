...
class DibiTracyPanel extends DibiObject implements IBarPanel
{
	...
	{
		if (is_callable('Tracy\Debugger::enable') && !class_exists('NDebugger')) {
			class_alias('Tracy\Debugger', 'NDebugger'); // PHP 5.2 code compatibility
		}
	...
	public function getPanel()
	{
		...
		foreach ($this->events as $event) {
			...
			if ($event->source) {
				$helpers = 'Tracy\Helpers';
				if (!class_exists($helpers)) {
					$helpers = class_exists('NDebugHelpers') ? 'NDebugHelpers' : 'DebugHelpers';
				}
				$s .= call_user_func(array($helpers, 'editorLink'), $event->source[0], $event->source[1]);
			}
